...
def _compute_rel_indices_3d(point_clouds):
    """
    Compute relative position encoding (dx, dy, dz, squared distance) for 3D point clouds.
    Matches ConViT paper Eq. 5 and convit2d: last component is ‖δ‖², not ‖δ‖.
    Used once per forward at the neck and passed into blocks to avoid incorrect caching.

    Args:
        point_clouds (torch.Tensor): (batch_size, num_points, 3) x,y,z coordinates.

    Returns:
        torch.Tensor: (batch_size, num_points, num_points, 4) (δx, δy, δz, ‖δ‖²).
    """
    ...
    distances_sq = (relative_positions ** 2).sum(dim=3, keepdim=True)
    return torch.cat((relative_positions, distances_sq), dim=3)
...
class GPSA(nn.Module):
    ...
    def forward(self, x, voxel_coord, rel_indices=None):
        "forward with scaled dot attention mechanism; rel_indices from neck when given"
        if rel_indices is None:
            rel_indices = _compute_rel_indices_3d(voxel_coord)
    ...
    def get_attention(self, x, rel_indices):
        B, N, C = x.shape
        ...
        pos_score = rel_indices
        pos_score = self.pos_proj(pos_score).permute(0,3,1,2)  # V_pos*(r)
        pos_score = pos_score.softmax(dim=-1)


        # Create identity on the same device/dtype as attention tensors.
        # We use it as the "value" so scaled_dot_product_attention outputs
        # the attention weights (shape: B x heads x N x N).
        I = torch.eye(k.shape[-2], k.shape[-2], device=q.device, dtype=q.dtype)
        ...
        # print("rel_indices shape", rel_indices.shape)
...
class MHSA(nn.Module):
    ...
    def forward(self, x, voxel_coords=None, rel_indices=None):
        B, N, C = x.shape
...
class Block(nn.Module):
    ...
    def forward(self, x, voxel_coords, rel_indices=None):
        x = x + self.drop_path(self.attn(self.norm1(x), voxel_coords, rel_indices))
