...
class IEVFE(nn.Module):
    ...
    Args:
        in_channels (int, optional): Input channels of VFE. Defaults to 4.
        feat_channels (list(int), optional): Channels of features in VFE, e.g IVFE=[32,128].
        with_distance (bool, optional): Whether to use the L2 distance
            of points to the origin point. Defaults to False.
        with_cluster_center (bool, optional): Whether to use the distance
            to cluster center of points inside a voxel. Defaults to False.
    ...
    def forward(self,
        ...
        # cancatinating the intensity histogram with IVFE features
        voxel_feats = torch.cat((voxel_feats,intensity_features ),1)
    ...
    def generateVoxelIntensityHist(self, features):      
        ...
        # [ΣK, 138] 
        # temp_hist_feature = features[:,:,3:4]
        temp_hist_feature = features[:,:,3]

   
        # print("shape of temp_hist_feature",temp_hist_feature.shape)
        final_feature = torch.zeros((features.shape[0],10),dtype=torch.float32).cuda()
