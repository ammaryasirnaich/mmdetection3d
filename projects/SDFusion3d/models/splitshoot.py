...
class LiftSplatShoot(nn.Module):
    ...
    def lift(self, image_features, intrinsics, extrinsics):
        ...
        depths = self.depths.to(image_features.device).view(1, 1, -1, 1)  # (1, 1, D, 1)
...
if __name__ == "__main__":
    ...
    model = LiftSplatShoot(depth_bins, H, W, N)

    # Run the forward pass
    point_cloud, BEV_grid = model(image_features, intrinsics, extrinsics)

    print(f'Point cloud shape: {point_cloud.shape}')  # Output point cloud shape   [B,N,C,H,W,D]
