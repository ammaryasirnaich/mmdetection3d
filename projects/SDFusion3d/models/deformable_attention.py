...
class DeformableAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Generate meshgrid of the original positions (h, w) once 
        if 'grid_h'not in self.meshgird or 'grid_w'not in self.meshgird:
            ...
            self.meshgird['grid_h']=grid_h
            self.meshgird['grid_w']=grid_w

        
        # Compute the reference points by adding the offsets
        ref_w = torch.clamp(self.meshgird['grid_w'] + offsets[:, :, 0, :, :], min=0, max=W-1)
        ref_h = torch.clamp(self.meshgird['grid_h'] + offsets[:, :, 1, :, :], min=0, max=H-1)
        ...
        ref_indices = torch.clamp(ref_indices, min=0, max=H*W-1).view(B, -1)  # [B, n_ref_points * H * W]
